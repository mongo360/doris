...
TEST_F(IcebergReaderEqualityDeleteTest, normal) {
    ...
    doris::RuntimeState _state(doris::TUniqueId(), doris::TQueryOptions(), doris::TQueryGlobals(),
                               nullptr);
    _state.init_mem_trackers();
    KVCache<std::string> _kv_cache;
    RuntimeProfile* _profile =
            _state.obj_pool()->add(new RuntimeProfile("IcebergReaderEqualityDeleteTest"));
    ...
    TDescriptorTable t_desc_table =
            apache::thrift::from_json_string<doris::TDescriptorTable>(t_desc_table_json);
    DescriptorTbl* desc_tbl;
    static_cast<void>(DescriptorTbl::create(_state.obj_pool(), t_desc_table, &desc_tbl));
    ...
    TFileScanRangeParams t_scan_param =
            apache::thrift::from_json_string<doris::TFileScanRangeParams>(scan_param_json);
    // with equality_detete to 0
    std::string scan_range_json =
            R"|({"2":{"str":"./be/test/exec/test_data/iceberg_reader/00001-0-3cdfaaeb-1a8a-4c75-9f86-61c3bd330d90-00415.parquet"},"3":{"i64":4},"4":{"i64":31489},"6":{"lst":["str",1,"2022-06"]},"7":{"lst":["str",1,"order_month"]},"8":{"rec":{"1":{"str":"iceberg"},"2":{"rec":{"1":{"i32":2},"2":{"i32":1},"3":{"lst":["rec",1,{"1":{"str":"./be/test/exec/test_data/iceberg_reader/00001-0-3cdfaaeb-1a8a-4c75-9f86-61c3bd330d90-00961.parquet"},"4":{"i32":2},"5":{"lst":["i32",4,1,34,2,101]}}]}}}}}})|";
    TFileRangeDesc t_scan_range =
            apache::thrift::from_json_string<doris::TFileRangeDesc>(scan_range_json);

    //std::unique_ptr<ParquetReader> parquet_reader = ParquetReader::create_unique(
    //        nullptr, t_scan_param, t_scan_range, 4000, &ctz, nullptr, nullptr);
    auto p_reader =
            new ParquetReader(nullptr, t_scan_param, t_scan_range, 4000, &ctz, nullptr, nullptr);
    static_cast<void>(p_reader->open());

    IcebergTableReader* iceberg_reader =
            new IcebergTableReader((GenericReader*)p_reader, _profile, &_state, t_scan_param,
                                   t_scan_range, _kv_cache, nullptr, 0);

    std::vector<std::string> file_col_names {"id", "order_id", "adowner_id", "order_time",
                                             "modify_time"};
    std::unordered_map<int, std::string> col_id_name_map {{1, "id"},
                                                          {2, "order_id"},
                                                          {34, "adowner_id"},
                                                          {75, "order_time"},
                                                          {79, "modify_time"}};
    ...
    colname_to_value_range["order_id"] = ColumnValueRange<TYPE_BIGINT>(
            "order_id", 258696459711L, 258696459712L, false, false, -1, -1);
    ...
    auto str_month = StringRef(month.c_str(), month.length());
    ColumnValueRange<TYPE_STRING>::add_fixed_value_range(
            std::get<7>(col_range_moth),
            reinterpret_cast<typename PrimitiveTypeTraits<TYPE_STRING>::CppType*>(&str_month));
    ...
    auto exprx = apache::thrift::from_json_string<doris::TExpr>(expr_json);
    ...
    state = iceberg_reader->init_reader(file_col_names, col_id_name_map, &colname_to_value_range,
                                        context);
    state = iceberg_reader->init_row_filters(t_scan_range);

    auto* partition_slot_desc =
            const_cast<doris::SlotDescriptor*>(desc_tbl->get_slot_descriptor(5));
